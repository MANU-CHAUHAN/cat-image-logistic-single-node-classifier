...
def final_model(X_train, Y_train, X_test, Y_test, iterations, learning_rate):
    """
    ...
    prediction_test = predict(w=w_optimized, b=b_optimized, X_new=X_test)

    prediction_train = predict(w=w_optimized, b=b_optimized, X_new=X_train)

    print('\n Accuracy for test set : ', np.mean(prediction_test == Y_test) * 100)

    print('\n Accuracy for train set : ', np.mean(prediction_train == Y_train) * 100)

    print('\n Try different values for learning_rate and iterations to change accuracy')
...
final_model(X_train=train_x, Y_train=train_set_y, X_test=test_x, Y_test=test_set_y, iterations=3000,
            learning_rate=0.001)
